// ========================================
// FRONTEND - CADASTRO ALUNO
// ========================================
// Este arquivo contém a lógica específica do cadastro de aluno
...
function cadastrarAluno() {
    ...
    // Validações básicas
    if (!alunoData.name || !alunoData.email || !alunoData.password) {
    ...
    if (alunoData.password.length < 6) {
        showAlert('A senha deve ter pelo menos 6 caracteres!', 'error');
        return;
    }

    // Verificar se o e-mail já existe
    if (users[alunoData.email]) {
    ...
    try {
        ...
        users[alunoData.email] = alunoData;
        ...
        showAlert('Erro ao cadastrar aluno. Tente novamente.', 'error');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (cpfResponsavelInput) {
        cpfResponsavelInput.addEventListener('input', function(e) {
            e.target.value = formatarCPF(e.target.value);
        });
        
        cpfResponsavelInput.addEventListener('blur', function(e) {
            const cpf = e.target.value.replace(/[^\d]/g, '');
            if (cpf.length === 11 && !validarCPF(cpf)) {
                showMessage(cpfResponsavelInput, 'CPF inválido!', 'error');
            } else if (cpf.length === 11) {
                showMessage(cpfResponsavelInput, 'CPF válido!', 'success');
            }
        });
    }
    ...
    if (dataNascimentoInput) {
        dataNascimentoInput.addEventListener('change', function(e) {
            ...
            if (idade < 5 || idade > 18) {
                showMessage(dataNascimentoInput, 'Idade deve estar entre 5 e 18 anos!', 'error');
            } else {
...
window.gerarMatricula = gerarMatricula;
